function[SvmOut,bias,alpha,lambda] = Train_Fun_SVM(C_plus,...
    C_minus,Sigma_param,train_matrix,train_label,MAxIter)
...
for i=1:tot_num_input_patterns
    for j =1:tot_num_input_patterns
        ...
        L(i,j)= train_label(i)*train_label(j)*(exp(-gamma*((train_matrix(i,:)-train_matrix(j,:))*(train_matrix(i,:)-train_matrix(j,:))')));
    end
end
...
%initialize H and f
HSvm =L ;
fSvm= -ones(tot_num_input_patterns,1); % Multiplying dual optimization problem by '-1'

%initialize Aeq, Beq in inequal constrain 
Aeq =train_label'; % making transpose to match the dimension with alpha
beq =0;

%initialize lower bound(lb) and upper bound(ub) in linear constrain
lb = zeros(tot_num_input_patterns,1);
%ub = ones(tot_num_input_patterns,1)*C; 
ub = ones(tot_num_input_patterns,1);
...
tic;
clear alpha
options = optimset('MaxIter',MAxIter); % how this iteration works inside the quadprog??
%%%x = quadprog(H,f,A,b,Aeq,beq,lb,ub,[],options);
[alpha,fval,exitflag,output,lambda] = quadprog(HSvm,fSvm,[],[],Aeq,beq,lb,ub,[],options);
toc;

%% SVMout Params
SVs_index = find(alpha>tolerance); % all non-zero value of alpha %% SVs index
SVs_coeff = alpha(alpha>tolerance); % vlaues of all non-zero alpha
...
for_calculating_b_pos_SVs_pattern_set_coef=  pos_SVs_pattern_set_coef(pos_SVs_pattern_set_coef < C_plus - tolerance);%excluding the outliers
